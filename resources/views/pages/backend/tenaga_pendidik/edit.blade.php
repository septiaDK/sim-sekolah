<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @endif
            <div>
                <form class="w-full" action="{{ route('tenaga_pendidik.update', [$tenaga_pendidik->id]) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-1/2 px-3">
                            ...
                            <input type="text" name="nama" placeholder="Ketik nama di sini"
                                ...
                                id="nama" value="{{ old('nama') ??  $tenaga_pendidik->nama }}" />
                        </div>
                        <div class="w-1/2 px-3">
                            <label for="nip"
                            ...
                            <input type="text" name="nip" placeholder="Ketik nip di sini"
                                class="appearance-none block w-full text-gray-800 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                id="nip" value="{{ old('nip') ?? $tenaga_pendidik->nip }}" />
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <label for="status"
                                ...
                                <select name="status" id="status" class="appearance-none block w-full text-gray-800 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                    ...
                                    <option value="kepala_sekolah" @selected($tenaga_pendidik->status == 'kepala_sekolah')>Kepala Sekolah</option>
                                    <option value="guru" @selected($tenaga_pendidik->status == 'guru')>Guru</option>
                                    <option value="staff karyawan" @selected($tenaga_pendidik->status == 'staff karyawan')>Staff Karyawan</option>
                                </select>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <span class="font-base inline-block text-xs text-red-600">Format : .jpg / .png /
                                .jpeg</span>
                            @if ($tenaga_pendidik->photo != null)
                                <img src="{{ url(Storage::url($tenaga_pendidik->photo)) }}"
                                    alt="photo" class="w-24 h-32 rounded-lg">
                            @endif
                    ...
                    <div class="flex flex-wrap -m-3 mb-6">
                        <div class="w-full px-3 text-right">
                            ...
                            <button type="submit"
                                class="shadow-lg bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Update</button>
                        </div>
                    </div>
                </form>
            </div>
