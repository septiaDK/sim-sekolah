<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @endif
            <div>
                <form class="w-full" action="{{ route('postingan.update', [$postingan->id]) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input type="text" name="judul" placeholder="Ketik judul di sini"
                                ...
                                id="judul" value="{{ old('judul') ?? $postingan->judul }}" />
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <label for="status"
                                ...
                                <select name="kategori_id" id="kategori_id" class="appearance-none block w-full text-gray-800 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                    ...
                                    @foreach ($categories as $value)
                                        @foreach ($postingan->categories as $categories)
                                            <option value="{{ $value->id }}" @selected($categories->id == $value->id)>{{ $value->nama }}</option>
                                        @endforeach
                                    @endforeach
                                </select>
                        </div>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <textarea name="isi" class="ckeditor appearance-none block w-full bg-white text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" type="text" placeholder="Ketik deskripsi di sini">{{ old('isi') ?? $postingan->isi }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <span class="font-base inline-block text-xs text-red-600">Format : .jpg / .png /
                                .jpeg</span>
                            @if ($postingan->cover != null)
                                <img src="{{ url(Storage::url($postingan->cover)) }}"
                    ...
                    <div class="flex flex-wrap -m-3 mb-6">
                        <div class="w-full px-3 text-right">
                            ...
                            <button type="submit"
                                class="shadow-lg bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Update Postingan</button>
                        </div>
                    </div>
                </form>
            </div>
